...
//! Le moteur de formatage d'entiers pese plusieurs Ko. Le boot LBA du Jour 17
//! leve le plafond, mais le formatage manuel reste le plus sur.

use crate::{serial, keyboard, timer, vga, vga2, scheduler};
...
impl Executor {
    ...
    fn draw_status_bar(&self, seconds: u64, ticks: u64) {
        ...
        line.push_dec(ticks);
        line.push_str(" | A ");
        line.push_dec(scheduler::counter_a());
        line.push_str(" | B ");
        line.push_dec(scheduler::counter_b());
        line.push_str(" | ");
    ...
    pub fn run(&mut self) -> ! {
        ...
        loop {
            ...
            if seconds != self.last_second {
                ...
                serial::print(" ticks - A=");
                serial::print_dec(scheduler::counter_a());
                serial::print(" B=");
                serial::print_dec(scheduler::counter_b());
                serial::print(" sw=");
                serial::print_dec(scheduler::switches());
                serial::print(" - ");
