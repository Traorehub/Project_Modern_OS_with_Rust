#!/bin/bash
# Jour 18 : assemblage boot LBA + helpers, puis kernel Rust.
set -e
cd "$(dirname "$0")"
...
objcopy -O binary "$KERNEL" /tmp/day18_size_check.bin

BYTES=$(stat -c%s /tmp/day18_size_check.bin)
SECTORS=$(( (BYTES + 511) / 512 ))
rm -f /tmp/day18_size_check.bin
