#!/bin/bash
# Jour 17 : demo graphique, fenetre QEMU (VGA + clavier PS/2)
...
if [ -n "$DISPLAY" ] || [ -n "$WAYLAND_DISPLAY" ]; then
  ...
  exec qemu-system-x86_64 \
    -name "Jour 17 - Boot LBA + contexte CPU" \
  ...
  echo "Sortie serie journalisee dans /tmp/day17_serial.log"
  ...
  exec qemu-system-x86_64 \
    -drive format=raw,file="$IMG" \
    -display curses \
    -serial "file:/tmp/day17_serial.log" \
    -no-reboot \
    -no-shutdown
fi
